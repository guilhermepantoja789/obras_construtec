<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-4">
                <a href="{{ route('propostas.show', $proposta) }}" class="p-2 bg-white/5 hover:bg-white/10 rounded-xl text-slate-400 transition-all border border-white/5">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                </a>
                <div>
                    <p class="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em] mb-0.5">Edição de Orçamento #{{ $proposta->id }}</p>
    ...
    <div class="max-w-7xl mx-auto px-4 pb-24" x-data="{
        items: {{ json_encode($proposta->items) }},
        addItem() {
        ...
        <form action="{{ route('propostas.update', $proposta) }}" method="POST" class="space-y-6 mt-4">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="titulo" value="{{ $proposta->titulo }}" required class="w-full bg-slate-900/50 border-white/10 rounded-2xl text-white text-sm focus:border-amber-500 py-3">
                    </div>
                    <div>
                        <label class="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Data</label>
                        <input type="date" name="data_proposta" value="{{ $proposta->data_proposta->format('Y-m-d') }}" required class="w-full bg-slate-900/50 border-white/10 rounded-2xl text-white text-sm focus:border-amber-500 py-3">
                    </div>
                    <div class="md:col-span-2">
                        <label class="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Escopo / Descrição</label>
                        <textarea name="escopo" rows="3" class="w-full bg-slate-900/50 border-white/10 rounded-2xl text-white text-sm focus:border-amber-500">{{ $proposta->escopo }}</textarea>
                    </div>
                    <div>
                        <label class="text-[10px] font-bold text-slate-500 uppercase mb-2 block">Status Atual</label>
                        <select name="status" class="w-full bg-slate-900/50 border-white/10 rounded-2xl text-white text-sm focus:border-amber-500 py-3">
                            ...
                            <option value="aceita" {{ $proposta->status == 'aceita' ? 'selected' : '' }}>Aceita (Gera Etapas)</option>
                            <option value="recusada" {{ $proposta->status == 'recusada' ? 'selected' : '' }}>Recusada</option>
                        </select>
                        @if($proposta->status == 'aceita')
                            <p class="text-[9px] text-amber-500/50 font-bold mt-2 uppercase tracking-tight">⚠️ Esta proposta já foi aceita. Alterações não gerarão novas etapas duplicadas.</p>
                        @endif
                    </div>
                </div>
            </div>
