                ...
                <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <x-input-label for="password" :value="__('Alterar Senha (deixe vazio para manter)')" />
                            ...
                            <div>
                                ...
                                <select id="role" name="role" class="mt-1 block w-full bg-white/5 border-white/10 focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm text-white backdrop-blur-sm">
                                    <option value="operador" class="bg-slate-900" {{ old('role', $user->role) == 'operador' ? 'selected' : '' }}>Operador de Obra (Preenche Diário)</option>
                                    <option value="cliente" class="bg-slate-900" {{ old('role', $user->role) == 'cliente' ? 'selected' : '' }}>Cliente (Visualiza Relatórios)</option>
                                </select>
                        ...
                        <div class="md:col-span-2 pt-6 border-t border-white/5">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">Controle as obras em andamento para este usuário.</p>
                                </div>
                                ...
                                @foreach($obras as $obra)
                                    <label class="obra-card relative flex flex-col p-4 bg-white/5 border border-white/10 rounded-2xl cursor-pointer hover:border-amber-500/50 transition-all group has-[:checked]:bg-amber-500/10 has-[:checked]:border-amber-500/50" data-nome="{{ strtolower($obra->nome) }}">
                                        <input type="checkbox" name="obras[]" value="{{ $obra->id }}" 
                                            {{ $user->obras->contains($obra->id) ? 'checked' : '' }}
                                            class="hidden peer">
