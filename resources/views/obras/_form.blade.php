@php
    $isEdit = isset($obra);
    $statusAtual = old('status', $isEdit ? $obra->status : 'em_andamento');
    $submitLabel = $isEdit ? __('Salvar Alterações') : __('Criar Obra');
    $formAction = $isEdit ? route('obras.update', $obra) : route('obras.store');

    $prazoExibicao = old('prazo_dias');
    if ($prazoExibicao === null && $isEdit) {
        $prazoExibicao = $obra->prazo_dias > 0 ? $obra->prazo_dias : '';
    }

    $prazoManualInicial = old('prazo_dias') !== null && old('prazo_dias') !== '';
    if (! $prazoManualInicial && $isEdit && $obra->prazo_dias > 0) {
...
<div class="max-w-4xl mx-auto -mx-4 sm:mx-auto" x-data="obraForm()">
    <div class="bg-white/5 sm:backdrop-blur-xl border-y sm:border border-white/10 overflow-hidden shadow-2xl sm:rounded-2xl">
        <form id="obra-form" action="{{ $formAction }}" method="POST"
              class="pb-28 sm:pb-0"
            ...
            <div class="p-4 sm:p-8 space-y-5 sm:space-y-8">
                ...
                <section class="space-y-4">
                    <div class="flex items-center gap-3 pb-1">
                        ...
                        <div>
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">Identificação</h3>
                            <p class="text-xs text-slate-600 hidden sm:block">Nome e status da obra</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 p-4 sm:p-5 bg-white/[0.03] rounded-2xl border border-white/5">
                        <div>
                            ...
                            <x-text-input id="nome" name="nome" type="text" class="mt-1 block w-full mobile-input" :value="old('nome', $isEdit ? $obra->nome : '')" required autofocus placeholder="Ex: Edifício Horizonte" />
                            <x-input-error class="mt-2" :messages="$errors->get('nome')" />
                        </div>

                        <div>
                            <x-input-label for="status" :value="$isEdit ? __('Status') : __('Status Inicial')" />
                            <select id="status" name="status" class="mobile-input mt-1 block w-full bg-white/5 border-white/10 focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm text-white backdrop-blur-sm">
                                <option value="planejamento" class="bg-slate-900" @selected($statusAtual === 'planejamento')>Planejamento</option>
                                <option value="em_andamento" class="bg-slate-900" @selected($statusAtual === 'em_andamento')>Em Andamento</option>
                                <option value="paralisada" class="bg-slate-900" @selected($statusAtual === 'paralisada')>Paralisada</option>
                                <option value="concluida" class="bg-slate-900" @selected($statusAtual === 'concluida')>Concluída</option>
                            </select>
                ...
                <section class="space-y-4">
                    <div class="flex items-center gap-3 pb-1">
                        ...
                        <div>
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">Endereço</h3>
                    ...
                    <div class="space-y-4 p-4 sm:p-5 bg-white/[0.03] rounded-2xl border border-white/5">
                        <div class="flex gap-2">
                            <div class="flex-1 min-w-0">
                                ...
                                <x-text-input id="cep" name="cep" type="text" inputmode="numeric" class="mt-1 block w-full mobile-input" :value="old('cep', $isEdit ? $obra->cep : '')" placeholder="00000-000" maxlength="9" />
                                <x-input-error class="mt-2" :messages="$errors->get('cep')" />
                                <p x-show="cepError" x-cloak x-text="cepError" class="mt-1.5 text-xs text-rose-400"></p>
                            </div>
                            <div class="flex items-end flex-shrink-0">
                                <button type="button" @click="buscarCep()" :disabled="cepLoading"
                                    class="h-12 px-4 bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all disabled:opacity-50 whitespace-nowrap">
                                    <span x-show="!cepLoading">Buscar</span>
                                    <span x-show="cepLoading" x-cloak>...</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="logradouro" name="logradouro" type="text" class="mt-1 block w-full mobile-input" :value="old('logradouro', $isEdit ? $obra->logradouro : '')" placeholder="Rua, Avenida..." />
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                            <div class="col-span-2 sm:col-span-1">
                                ...
                                <x-text-input id="bairro" name="bairro" type="text" class="mt-1 block w-full mobile-input" :value="old('bairro', $isEdit ? $obra->bairro : '')" />
                                <x-input-error class="mt-2" :messages="$errors->get('bairro')" />
                            </div>
                            <div>
                                <x-input-label for="cidade" :value="__('Cidade')" />
                                <x-text-input id="cidade" name="cidade" type="text" class="mt-1 block w-full mobile-input" :value="old('cidade', $isEdit ? $obra->cidade : '')" />
                                <x-input-error class="mt-2" :messages="$errors->get('cidade')" />
                            </div>
                            <div>
                                <x-input-label for="estado" :value="__('UF')" />
                                <x-text-input id="estado" name="estado" type="text" class="mt-1 block w-full mobile-input uppercase" :value="old('estado', $isEdit ? $obra->estado : '')" maxlength="2" placeholder="AM" />
                        ...
                        <div>
                            ...
                            <x-text-input id="localizacao" name="localizacao" type="text" class="mt-1 block w-full mobile-input" :value="old('localizacao', $isEdit ? $obra->localizacao : '')" placeholder="Ex: Próximo ao metrô" />
                ...
                <section class="space-y-4">
                    <div class="flex items-center gap-3 pb-1">
                        ...
                        <div>
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">Prazos</h3>
                    ...
                    <div class="space-y-4 p-4 sm:p-5 bg-white/[0.03] rounded-2xl border border-white/5">
                        <div class="grid grid-cols-2 gap-3 sm:gap-4">
                            <x-date-br-input
                                ...
                                :value="old('data_inicio', $isEdit && $obra->data_inicio ? $obra->data_inicio->format('Y-m-d') : '')"
                            />
                            <x-date-br-input
                                ...
                                :optional="true"
                                inputClass="mobile-input"
                                :value="old('data_fim_prevista', $isEdit && $obra->data_fim_prevista ? $obra->data_fim_prevista->format('Y-m-d') : '')"
                            />
                        </div>
                        <p x-show="dateError" x-cloak x-text="dateError" class="text-xs text-rose-400"></p>

                        <div>
                            <x-input-label for="prazo_dias" :value="__('Prazo Total (Dias)')" />
                            <x-text-input id="prazo_dias" name="prazo_dias" type="number" inputmode="numeric" min="0" class="mt-1 block w-full mobile-input" :value="$prazoExibicao" placeholder="Ex: 180" />
                            ...
                            <p class="mt-1.5 text-xs text-slate-500">Opcional — calculado pelas datas quando início e término estiverem preenchidos</p>
                ...
                <section class="space-y-4">
                    <div class="flex items-center gap-3 pb-1">
                        ...
                        <div>
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest">Contratantes</h3>
                    ...
                    <div class="space-y-4 p-4 sm:p-5 bg-white/[0.03] rounded-2xl border border-white/5">
                        <div>
                            ...
                            <x-text-input id="contratante" name="contratante" type="text" class="mt-1 block w-full mobile-input" :value="old('contratante', $isEdit ? $obra->contratante : '')" placeholder="Ex: Nome da Empresa ou Cliente" />
                            <x-input-error class="mt-2" :messages="$errors->get('contratante')" />
                        </div>
                        <div>
                            <x-input-label for="cnpj_contratante" :value="__('CNPJ do Contratante')" />
                            <x-text-input id="cnpj_contratante" name="cnpj_contratante" type="text" inputmode="numeric" class="mt-1 block w-full mobile-input cnpj-mask" :value="old('cnpj_contratante', $isEdit ? $obra->cnpj_contratante : '')" placeholder="00.000.000/0000-00" />
                        ...
                        <div class="border-t border-white/5 pt-4">
                            ...
                            <x-text-input id="empresa_contratada" name="empresa_contratada" type="text" class="mt-1 block w-full mobile-input" :value="old('empresa_contratada', $isEdit ? $obra->empresa_contratada : '')" placeholder="Ex: Nome da sua Construtora" />
                        ...
                        <div>
                            ...
                            <x-text-input id="cnpj_empresa_contratada" name="cnpj_empresa_contratada" type="text" inputmode="numeric" class="mt-1 block w-full mobile-input cnpj-mask" :value="old('cnpj_empresa_contratada', $isEdit ? $obra->cnpj_empresa_contratada : '')" placeholder="00.000.000/0000-00" />
                        ...
                        <div class="border-t border-white/5 pt-4">
                            ...
                            <x-text-input id="engenheiro_responsavel" name="engenheiro_responsavel" type="text" class="mt-1 block w-full mobile-input" :value="old('engenheiro_responsavel', $isEdit ? $obra->engenheiro_responsavel : '')" placeholder="Ex: Ronaldo Souza 24958-AM" />
        ...
        return {
            ...
            handleSubmit(event) {
                ...
                if (!result.ok) {
                    ...
                    result.input.focus();
                    this.dateError = 'Informe uma data válida no formato DD/MM/AAAA.';
                    return;
                }
            ...
            buscarCep() {
                ...
                if (cep.length !== 8) {
                    this.cepError = 'Informe um CEP válido com 8 dígitos.';
                    return;
                }
                ...
                fetch(`https://viacep.com.br/ws/${cep}/json/`)
                    ...
                    .catch(() => {
                        this.cepError = 'Não foi possível consultar o CEP. Tente novamente.';
                    })
                    .finally(() => {
